...
public final class ImageEnhancer {
    ...
    /**
     * Brightness + contrast adjustment (single pass).
     * @param brightnessOffset -100..100 (0 = no change)
     * @param contrastFactor   0.2..2.0 (1.0 = no change)
     */
    public static Bitmap applyBrightnessContrast(Bitmap src, int brightnessOffset,
                                                 float contrastFactor) {
        if (brightnessOffset == 0 && Math.abs(contrastFactor - 1f) < 0.001f) return src;
        ...
        for (int i = 0; i < px.length; i++) {
            int c = px[i];
            int r = adj((c >> 16) & 0xff, brightnessOffset, contrastFactor);
            int g = adj((c >> 8) & 0xff, brightnessOffset, contrastFactor);
            int b = adj(c & 0xff, brightnessOffset, contrastFactor);
            out[i] = 0xFF000000 | (r << 16) | (g << 8) | b;
        }
        return Bitmap.createBitmap(out, w, h, Bitmap.Config.ARGB_8888);
    }

    private static int adj(int v, int offset, float contrast) {
        int r = Math.round((v - 128) * contrast + 128 + offset);
        return r < 0 ? 0 : (r > 255 ? 255 : r);
    }

    /** Convenience: sharpness first, then brightness/contrast. */
    public static Bitmap applyAll(Bitmap base, int sharpnessPercent,
                                  int brightnessOffset, float contrastFactor) {
        Bitmap b = unsharpMask(base, sharpnessPercent);
        return applyBrightnessContrast(b, brightnessOffset, contrastFactor);
    }
